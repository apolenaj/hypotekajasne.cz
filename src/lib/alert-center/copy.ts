...
export function buildLtvRateChangeAlert(input: {
  ...
  return {
    ...
    expiresAt,
    dataSource: {
      module: "Veřejné sazby sledovaných bank",
      recordId: "rate.cz.market.aggregate",
      claimKind: "DATA",
      status: "LIVE",
      fetchedAt: input.fetchedAt,
    },
    whyExplanation: `Sledujeme změnu referenční sazby v pásmu odpovídajícím vašemu modelovému LTV (~${Math.round(input.ltvPercent)} %) a záměru ${input.purpose === "investment" ? "investice" : "vlastní bydlení"}. Toto upozornění není individuální nabídka banky.`,
...
export function buildFixationAlert(input: {
  ...
  return {
    ...
    whyExplanation: `Máte uložený profil hypotéky s datem fixace. Upozornění je personalizované k vaší splátce — ne generické „sazby klesly“.`,
...
export function buildPropertyPriceAlert(input: {
  ...
  return {
    ...
    dataSource: {
      ...
      status: input.claimKind === "DATA" ? "LIVE" : "MODEL",
      fetchedAt: now.toISOString(),
    },
    whyExplanation: `Sledujete tuto nemovitost a zaznamenali jsme změnu pozorované ceny ≥ 50 000 Kč.`,
    fingerprint: buildFingerprint(
...
export function buildDocumentExpiryAlert(input: {
  ...
  return {
    ...
    dataSource: {
      module: "Dokumentový trezor",
      recordId: input.documentId,
      claimKind: "DATA",
      status: "VERIFIED",
      fetchedAt: now.toISOString(),
    },
    whyExplanation: `Dokument je ve vašem dokumentovém trezoru s vyplněnou expirací. Jde o připomínku z vašich podkladů — ne o právní závěr o platnosti.`,
    fingerprint: buildFingerprint(
      "DOCUMENT_EXPIRY",
      input.documentId,
...
export function buildRegulatoryAlert(input: {
  ...
  return {
    ...
    dataSource: {
      module: "Trhový puls — regulace",
      recordId: input.entryId,
      claimKind: "DATA",
      status: "VERIFIED",
      fetchedAt: now.toISOString(),
    },
    whyExplanation: `Sledujete Český trh nebo máte aktivní hypoteční profil — regulační změna může ovlivnit rámec LTV/DTI. Jde o ověřené záznamy, ne o individuální doporučení.`,
...
export function buildDealTaskAlert(input: {
  ...
  return {
    ...
    reason: input.dueAt
      ...
      : "Otevřený úkol ve pracovním prostoru transakce.",
    action: input.action,
    expiresAt: input.dueAt,
    dataSource: {
      module: "Transakční místnost",
...
export function buildPortfolioRiskAlert(input: {
  ...
  return {
    ...
    dataSource: {
      module: "Přehled portfolia",
      recordId: input.alertId,
      claimKind: "MODEL",
      status: "MODEL",
      fetchedAt: now.toISOString(),
    },
    whyExplanation: `Máte nemovitosti ve správě portfolia — koncentrace nebo riziko je spočítáno z vámi zadaných hodnot (orientační modelový výpočet).`,
