export const FAQ_ITEMS = [
  {
    q: "Poskytujete přímo hypotéky nebo finanční poradenství?",
    a: "Ne. Hypotéka Jasně je technologická a informační platforma — edukace, kalkulačky a modely. Nejsme banka. Individuální zprostředkování dělá licencovaný specialista; schválení úvěru vždy banka. Role jsou popsány v Trust Center (/duvera).",
  },
  {
    q: "Jsou vaše kalkulace a investiční skóre závazné?",
    a: "Ne. Výpočty na webu jsou modelové a orientační. Skutečné sazby, poplatky a výnosy se liší. Závaznou nabídku vydává banka (případně po práci se specialistou).",
  },
  {
    q: "Platím něco za využívání kalkulaček?",
    a: "Používání nástrojů na webu je pro vás zdarma. Můžeme získat odměnu od partnera, pokud přes nás dojde k realizaci. Detail: /jak-vydelavame.",
  },
  {
    q: "Komu předáváte mé kontakty?",
    a: "Jen pokud o handoff požádáte. Předáváme licencovanému hypotečnímu partnerovi — IČO, role a JERRS odkaz (po ověření) najdete na /partneri. Majetio, makléř nebo developer jsou oddělené role.",
  },
] as const;
