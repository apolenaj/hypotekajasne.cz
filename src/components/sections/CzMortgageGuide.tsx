...
import {
  czGuideArticlesData,
  type CzGuideArticle,
} from "@/lib/cz-guide-articles-data";
...
function GuideArticleCard({
  ...
  article: CzGuideArticle;
  onSelect: (article: CzGuideArticle) => void;
}) {
  return (
    <article
      role="button"
...
function RedFlagsSection() {
  const redFlags = countryGuides.cz.redFlags;
...
export function CzMortgageGuide() {
  const [selectedGuideArticle, setSelectedGuideArticle] =
    useState<CzGuideArticle | null>(null);

  const handleBack = () => setSelectedGuideArticle(null);

  useEffect(() => {
    if (selectedGuideArticle) {
      document.getElementById("cz-mortgage-guide")?.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    }
  }, [selectedGuideArticle]);

  return (
    <section
      id="cz-mortgage-guide"
          ...
          <>
            <div className="text-center mb-12">
              ...
              <h2 className="font-heading text-4xl md:text-5xl font-extrabold text-gray-900 mt-2 mb-4">
                Průvodce hypotékou v ČR
              </h2>
              <p className="text-gray-600 text-lg">
                Vše, co potřebujete vědět před podpisem smlouvy
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
              {czGuideArticlesData.map((article) => (
                <GuideArticleCard
                  key={article.id}
                  article={article}
                  onSelect={setSelectedGuideArticle}
                />
              ))}
            </div>
